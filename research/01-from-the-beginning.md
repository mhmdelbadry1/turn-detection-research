...
Dates are 2026. **Fact** = measured or read from source. **As reported** = production state described in an internal 2026-09-01 note, not re-checked against the cluster in this work. **Opinion** = interpretation.
...
Customers barge in, fill, and hold. NAMO cuts them off or waits too long. The audio pause model under consideration was **Smart Turn v3.2** (BSD-2, 8 MB ONNX, trainable). The accuracy winner in public numbers was **LiveKit v1-mini**, usable only inside LiveKit Agents 1.6.x (production pin: 1.4.5).
...
6. **Score-fusion of VAP + Smart Turn on eot-bench.** Hurt Arabic.
...
- No 300–500 Gulf export (not available for this work).
